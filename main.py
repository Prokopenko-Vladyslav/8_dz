...
def get_birthdays_per_week(users):
    ...
    for user in users:
        ...
        if days_until_birthday <= 6:
            ...
            day_of_week = (
                today + timedelta(days_until_birthday)
                ).strftime('%A')
            ...
            if day_of_week in birthday_holiday:
                ...
                if birthday_holiday:
                    if 'Sunday' in birthday_holiday:
                        birthday_dict['Monday'].extend(
                            birthday_holiday["Sunday"]
                            )
                    if 'Saturday' in birthday_holiday:
                        birthday_dict['Monday'].extend(
                            birthday_holiday['Saturday']
                            )
    ...
    if not any(birthday_dict.values()):
        ...
        non_empty_days = {
            day: users for day, users in birthday_dict.items() if users
            }
